...
function tokenize(source) {
  ...
  while (i < n) {
    ...
    if (/[0-9]/.test(c) || (c === '.' && /[0-9]/.test(peek(1)))) {
      ...
      // 小数点は次が数字のときだけ（1..3 の range を壊さない）
    ...
    if (/[A-Za-z_]/.test(c)) {
      ...
      if (KEYWORDS.has(id)) {
        if (id === 'true' || id === 'false') tokens.push({ type: 'bool', value: id === 'true', line: startLine })
        else if (id === 'null') tokens.push({ type: 'null', value: null, line: startLine })
        else tokens.push({ type: id, line: startLine })
      } else tokens.push({ type: 'ident', value: id, line: startLine })
      continue
    }
    const two = c + peek(1)
    const TWO = {
    ...
    const SINGLE = {
      ...
      '[': 'lbracket', ']': 'rbracket', ',': 'comma', '=': 'assign',
      '<': 'lt', '>': 'gt', '!': 'not', ';': 'semi', ':': 'colon',
    }
    if (SINGLE[c]) {
      const startLine = line
      adv()
...
function parse(tokens) {
  ...
  function parseStmt() {
    ...
    if (match('assign')) {
      if (expr.type !== 'Ident' && expr.type !== 'Index') throw new RuntimeError('invalid assignment target', peek().line)
      const value = parseExpr()
      match('semi')
      return { type: 'Assign', target: expr, value, line: expr.line }
    }
    for (const [op, name] of [['plusEq', '+'], ['minusEq', '-'], ['starEq', '*'], ['slashEq', '/']]) {
      if (match(op)) {
        if (expr.type !== 'Ident') throw new RuntimeError('invalid assignment target', peek().line)
        const value = parseExpr()
        match('semi')
        return { type: 'Assign', target: expr, value: { type: 'Binary', op: name, left: expr, right: value, line: expr.line }, line: expr.line }
      }
    }
    match('semi')
  ...
  function parseIf() {
    ...
    let els = null
    if (match('else')) els = at('if') ? parseIf() : parseBlock()
  ...
  function parseReturn() {
    ...
    let value = null
    if (!at('rbrace') && !at('eof') && !at('semi')) value = parseExpr()
    match('semi')
  ...
  function parseOr() {
    let left = parseAnd()
    while (match('or')) left = { type: 'Binary', op: '||', left, right: parseAnd(), line: left.line }
    return left
  }
  function parseAnd() {
    let left = parseEquality()
    while (match('and')) left = { type: 'Binary', op: '&&', left, right: parseEquality(), line: left.line }
    return left
  }
  ...
  function parseRange() {
    let left = parseTerm()
    if (match('range')) return { type: 'Range', start: left, end: parseTerm(), line: left.line }
    return left
  }
  function parseTerm() {
  ...
  function parseUnary() {
    if (match('minus')) { const arg = parseUnary(); return { type: 'Unary', op: '-', arg, line: arg.line } }
    if (match('not')) { const arg = parseUnary(); return { type: 'Unary', op: '!', arg, line: arg.line } }
    return parseCall()
  }
  function parseCall() {
    ...
    while (true) {
      if (match('lparen')) {
        const args = []
        if (!at('rparen')) { args.push(parseExpr()); while (match('comma')) args.push(parseExpr()) }
        consume('rparen')
  ...
  function parsePrimary() {
    ...
    if (match('lparen')) { const e = parseExpr(); consume('rparen'); return e }
    if (match('lbracket')) {
      const elements = []
      if (!at('rbracket')) { elements.push(parseExpr()); while (match('comma')) elements.push(parseExpr()) }
      consume('rbracket')
...
class Environment {
  ...
  define(name, value) { this.map[name] = value }
  get(name, line) {
  ...
  set(name, value, line) {
    if (Object.prototype.hasOwnProperty.call(this.map, name)) { this.map[name] = value; return }
  ...
  entries() {
    const out = {}
    let env = this
    while (env) { Object.assign(out, env.map); env = env.parent }
    return out
  }
}
...
function interpolate(str, env) {
  return String(str).replace(/\$\{([^}]+)\}/g, (_, raw) => {
    try { return repr(env.get(raw.trim(), 0)) } catch { return '${' + raw.trim() + '}' }
  })
}

function makeBuiltins(envRoot) {
  return {
    len: (a) => {
    ...
    env: () => 'lang=' + navigator.language + ' online=' + navigator.onLine + ' ua=' + navigator.userAgent.slice(0, 48) + '…',
    push: (arr, v) => { if (!Array.isArray(arr)) throw new RuntimeError('push() expects array'); arr.push(v); return arr.length },
    pop: (arr) => { if (!Array.isArray(arr)) throw new RuntimeError('pop() expects array'); return arr.pop() },
    join: (arr, sep = ',') => { if (!Array.isArray(arr)) throw new RuntimeError('join() expects array'); return arr.map(repr).join(String(sep)) },
...
function evaluate(node, env, ctx) {
  if (!node) return null
  switch (node.type) {
    case 'Program': { let last = null; for (const s of node.body) last = evaluate(s, env, ctx); return last }
    case 'Block': {
    ...
    case 'Let': { const v = evaluate(node.value, env, ctx); env.define(node.name, v); return v }
    case 'Assign': {
      const v = evaluate(node.value, env, ctx)
      if (node.target.type === 'Ident') { env.set(node.target.name, v, node.line); return v }
    ...
    case 'Fn': {
      const fn = { __fn: true, name: node.name, params: node.params, body: node.body, closure: env }
    ...
    case 'Print': {
      const parts = node.args.map((a) => {
        ...
        return typeof v === 'string' ? interpolate(v, env) : repr(v)
      })
      ctx.print(parts.join(' '))
      return null
    }
    case 'ExprStmt': return evaluate(node.expr, env, ctx)
    case 'Literal': return node.value
    case 'Ident': return env.get(node.name, node.line)
    case 'Array': return node.elements.map((e) => evaluate(e, env, ctx))
    case 'Range': {
      return { __range: true, start: Number(evaluate(node.start, env, ctx)), end: Number(evaluate(node.end, env, ctx)) }
    }
    case 'Unary': {
      const a = evaluate(node.arg, env, ctx)
      if (node.op === '-') return -Number(a)
      if (node.op === '!') return !isTruthy(a)
      throw new RuntimeError('unknown unary', node.line)
    }
    case 'Binary': {
      if (node.op === '&&') { const l = evaluate(node.left, env, ctx); return isTruthy(l) ? evaluate(node.right, env, ctx) : l }
      if (node.op === '||') { const l = evaluate(node.left, env, ctx); return isTruthy(l) ? l : evaluate(node.right, env, ctx) }
      ...
      switch (node.op) {
        ...
        case '*': return Number(l) * Number(r)
        case '/': if (Number(r) === 0) throw new RuntimeError('division by zero', node.line); return Number(l) / Number(r)
        case '%': return Number(l) % Number(r)
        case '==': return l === r
        case '!=': return l !== r
        case '<': return l < r
        ...
        case '<=': return l <= r
        case '>=': return l >= r
        default: throw new RuntimeError('unknown op', node.line)
      }
    }
    case 'Call': {
      ...
      if (callee && callee.__fn) {
        if (args.length !== callee.params.length) {
          throw new RuntimeError('fn ' + callee.name + ' expects ' + callee.params.length + ' args, got ' + args.length, node.line)
        }
        const local = new Environment(callee.closure)
        callee.params.forEach((p, i) => local.define(p, args[i]))
        try { evaluate(callee.body, local, ctx); return null }
    ...
    default: throw new RuntimeError('unknown node ' + node.type, node.line || 0)
  }
}

export function runPilotScript(source, options = {}) {
  const output = []
  const print = (s) => { output.push(String(s)); if (options.print) options.print(String(s)) }
  try {
    const tokens = tokenize(source)
    const ast = parse(tokens)
    const env = new Environment()
    const builtins = makeBuiltins(env)
    for (const [k, v] of Object.entries(builtins)) env.define(k, v)
    env.define('help', () => { print(HELP_TEXT); return null })
...
export const HELP_TEXT = [
  ...
  'if cond { } else { }',
  'while cond { }',
  'for i in 1..5 { }   |  for item in arr { }',
  'fn add(a, b) { return a + b }',
  'let a = [1,2,3]  |  a[0]  |  push(a, 4)',
  'print "hi ${name}"  |  operators: + - * / % == != < > and or not',
  'builtins: len type str num abs floor ceil min max now env push pop join keys help()',
  'comments: # ...  or  // ...',
].join('\n')
